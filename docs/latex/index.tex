...
This overview shall give you first impression what to expect from each of the examples. The background/basics group gives you the promised look under the hood. Whereas the application group shows you how you can use the \hyperlink{namespaceSacado__Wrapper}{Sacado\+\_\+\+Wrapper} to quickly compute tangents. 
...
\item[\hyperlink{todo__todo000001}{Todo}]Check if factor 0.\+5 is also necessary for d\+\_\+sigma / d\+\_\+phi\end{DoxyRefDesc}
...
\item[\hyperlink{todo__todo000002}{Todo}]clean up the documentation of the classes\end{DoxyRefDesc}
...
\end{DoxyCode}
 \begin{DoxyRefDesc}{Todo}
\item[\hyperlink{todo__todo000003}{Todo}]use boldsymbol for tensors\end{DoxyRefDesc}
...
\end{DoxyCode}
 \begin{DoxyRefDesc}{Todo}
\item[\hyperlink{todo__todo000004}{Todo}]It would be nice to use the data types from the \hyperlink{namespaceSacado__Wrapper}{Sacado\+\_\+\+Wrapper} for all the Sacado variables. But somehow the operators (multiply$\ast$, ...) seem to cause conflicts again.\end{DoxyRefDesc}
...
\end{DoxyCode}
 Be aware of the difference between \[ eps_d \dyadic \boldsymbol{1} \text{ and } \boldsymbol{1} \dyadic eps_d \] 
\begin{DoxyCode}
std::cout << \textcolor{stringliteral}{"C\_analy ="} << C\_analy << std::endl << std::endl;
\end{DoxyCode}
...
\item[\hyperlink{todo__todo000005}{Todo}]Maybe add some more text on the linearization \end{DoxyRefDesc}
...
\item[\hyperlink{todo__todo000006}{Todo}]try to merge the fad\+\_\+double data type with this templated data type \end{DoxyRefDesc}
...
\{
    ...
    \textcolor{keywordflow}{for} ( \textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} i=0; i<dim; ++i)
        \textcolor{keywordflow}{for} ( \textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} j=0; j<dim; ++j )
        \{
            \textcolor{keywordtype}{double} *derivs = &sigma[i][j].fastAccessDx(0); \textcolor{comment}{// Access the derivatives of the (i,j)-th
    ...
    std::cout << \textcolor{stringliteral}{"dev="} << deviator(eps\_d) << std::endl;
 
    std::cout << \textcolor{stringliteral}{"C\_Sacado*eps\_d="} << stress\_from\_tangent << std::endl;
    std::cout << stress\_from\_tangent[0][1] << std::endl;
    std::cout << stress\_from\_tangent[0][2] << std::endl;
    std::cout << stress\_from\_tangent[1][2] << std::endl;
\end{DoxyCode}
 \begin{DoxyRefDesc}{Todo}
\item[\hyperlink{todo__todo000007}{Todo}]Check this in detail and finish it \end{DoxyRefDesc}
